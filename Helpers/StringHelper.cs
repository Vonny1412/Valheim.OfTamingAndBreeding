...
namespace OfTamingAndBreeding.Utils
{
    ...
    internal static class StringHelper
    {
            ...
            string colorNegative)
        {
            ...
            if (ts.Days > 0)
                timeString += String.Format(L.Localize("$tmt_hover_timer_days"), color, ts.Days);
            if (ts.Hours > 0 || timeString != "")
                timeString += String.Format(L.Localize("$tmt_hover_timer_hours"), color, ts.Hours);
            if (ts.Minutes > 0 || timeString != "")
                timeString += String.Format(L.Localize("$tmt_hover_timer_minutes"), color, ts.Minutes);
            if (Plugin.Configs.HoverShowSeconds.Value)
                timeString += String.Format(L.Localize("$tmt_hover_timer_seconds"), color, ts.Seconds);
