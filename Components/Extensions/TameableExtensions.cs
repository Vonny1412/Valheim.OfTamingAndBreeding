...
namespace OfTamingAndBreeding.Components.Extensions
{
    internal static class TameableExtensions
    {
        ...
        public static ZNetView GetZNetView(this Tameable that)
            => ValheimAPI.Tameable.__IAPI_m_nview_Invoker.Get(that);
        ...
        public static Character GetCharacter(this Tameable that)
            => ValheimAPI.Tameable.__IAPI_m_character_Invoker.Get(that);
        ...
        public static void Awake_PatchPostfix(this Tameable tameable)
        {
            ...
            if (tameable.TryGetComponent<OTAB_CustomAnimalAI>(out var customAnimalAI))
            {
            ...
            if (tameable.TryGetComponent<OTAB_TameableTrait>(out _) == false)
            {
                // we are using late-registration
                // instead of adding component in CreatureProcessor

                var prefabName = global::Utils.GetPrefabName(tameable.gameObject.name);
                var prefab =  PrefabManager.Instance.GetPrefab(prefabName);
                var prefabTameable = prefab.GetComponent<Tameable>();

                var c1 = tameable.gameObject.gameObject.AddComponent<OTAB_TameableTrait>();
                var c2 = prefab.gameObject.AddComponent<OTAB_TameableTrait>();
        ...
        private static void UpdateTamingTime(this Tameable tameable, float totalFactor)
        {
            ...
            {
                var trait = tameable.GetComponent<OTAB_TameableTrait>();
        ...
        private static void UpdateFedDuration(this Tameable tameable, float totalFactor)
        {
            if (totalFactor >= 0) // yes, we do allow 0, too
            {
                var baseValues = tameable.GetComponent<OTAB_TameableTrait>();
        ...
        {
            if (tameable.TryGetComponent<OTAB_Creature>(out var creature))
            {
        ...
        public static bool IsStarving(this Tameable tameable)
        {
            ...
            if (tameable.TryGetComponent<OTAB_Creature>(out var creatureTrait))
            {
        ...
        {
            var creature = tameable.GetComponent<OTAB_Creature>();
