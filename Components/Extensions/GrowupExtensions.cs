...
namespace OfTamingAndBreeding.Components.Extensions
{
    internal static class GrowupExtensions
    {
        ...
        public static ZNetView GetZNetView(this Growup that)
            => ValheimAPI.Growup.__IAPI_m_nview_Invoker.Get(that);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static BaseAI GetBaseAI(this Growup that)
            => ValheimAPI.Growup.__IAPI_m_baseAI_Invoker.Get(that);
        ...
        public static void Start_PatchPostfix(this Growup growup)
        {
            ...
            if (growup.TryGetComponent<OTAB_GrowupTrait>(out _) == false)
            {
                // we are using late-registration
                // instead of adding component in CreatureProcessor

                var prefabName = global::Utils.GetPrefabName(growup.gameObject.name);
                var prefab = PrefabManager.Instance.GetPrefab(prefabName);
                var prefabGrowup = prefab.GetComponent<Growup>();

                var c1 = growup.gameObject.gameObject.AddComponent<OTAB_GrowupTrait>();
                var c2 = prefab.gameObject.AddComponent<OTAB_GrowupTrait>();
        ...
        private static void UpdateGrowTime(this Growup growup, float totalFactor)
        {
            ...
            {
                var trait = growup.GetComponent<OTAB_GrowupTrait>();
